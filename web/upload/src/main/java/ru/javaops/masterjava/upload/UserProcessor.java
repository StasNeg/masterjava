...
public class UserProcessor {
    ...
    public List<FailedEmails> process(final StaxStreamProcessor processor, Map<String, Group> groups, Map<String, City> cities, int chunkSize) throws XMLStreamException, JAXBException {
        ...
        while (processor.doUntil(XMLEvent.START_ELEMENT, "User")) {
            ...
            String groupUser = processor.getAttribute("groupRefs");  // unmarshal doesn't get group groupRefs
            List<String> groupUserList = getStrings(groupUser);
            ru.javaops.masterjava.xml.schema.User xmlUser = unmarshaller.unmarshal(processor.getReader(), ru.javaops.masterjava.xml.schema.User.class);
            if (Collections.disjoint(groups.keySet(), groupUserList)) {
                failed.add(new FailedEmails(xmlUser.getEmail(), "Groups '" + groupUser + "' is not present in DB"));
            } else if (cities.get(cityRef) == null) {
                failed.add(new FailedEmails(xmlUser.getEmail(), "City '" + cityRef + "' is not present in DB"));
            } else {
                final User user = new User(id++, xmlUser.getValue(), xmlUser.getEmail(), UserFlag.valueOf(xmlUser.getFlag().value()), cityRef, getGroupId(groupUser, groups));
                chunk.add(user);
...
//        List<FailedEmails> failed = new ArrayList<>();
    ...
    private Set<Integer> getGroupId(String groupUser, Map<String, Group> groups) {
        return getStrings(groupUser)
                .stream()
                .map(x -> groups.get(x).getId())
                .collect(Collectors.toSet());
    }

    private List<String> getStrings(String groupUser) {
        return Splitter.on(' ').splitToList(nullToEmpty(groupUser));
    }

...
public class User {
    ...
    public List<String> groupsToGroupsId(){
        return group.stream().map(x->((GroupSimple)x.getValue()).getId()).collect(Collectors.toList());
    }
    public String cityToString(){
        return ((CityType)city).getValue();
    }
}
